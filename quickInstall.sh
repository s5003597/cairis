...
git clone -b stixconversion https://github.com/s5003597/cairis $CAIRIS_ROOT
...
echo "alias update_cairis=\"sudo apt-get update && sudo apt-get upgrade -y && sudo apt-get dist-upgrade -y && curl -s https://raw.githubusercontent.com/s5003597/cairis/stixconversion/quickInstall.sh | bash -s $ROOTPW\"" >> $HOME/.bashrc
